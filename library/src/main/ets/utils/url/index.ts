...
import { url as arktsUrl } from '@kit.ArkTS';
 ...
 */
export function copyUrl(original: arktsUrl.URL): arktsUrl.URL {
	return new arktsUrl.URL(original.toString());
}
 ...
 */
export function hasProtocolAndPort(urlString: string, url: arktsUrl.URL): boolean {
 ...
 */
export function isDefaultPort(url: arktsUrl.URL): boolean {
	return !url.port
 ...
 */
export function parseUrl(input: string): arktsUrl.URL {
	if (!input?.trim()) {
		throw new Error('Input is required');
	}

	return new arktsUrl.URL(
		normalizeUrl(input)
	);
}
